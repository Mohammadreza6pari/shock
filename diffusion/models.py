...
class Diffusion(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="diffusions"
    )
    ...
    def output_folder_path_exists(self):
        return os.path.exists(self.output_folder_path) and os.path.isdir(
    ...
    def cmd_arguments(self):
        command_args = [
            ...
            Diffusion.to_cmd_argument_fields(
                self.sources
            ),  # Sources as a colon-separated string
            Diffusion.to_cmd_argument_fields(
                self.destinations
            ),  # Destinations as a colon-separated string
            Diffusion.to_cmd_argument_fields(
                self.shock_types
            ),  # Shock types as a colon-separated string
            Diffusion.to_cmd_argument_fields(
                self.shock_amounts
            ),  # Shock amounts as a colon-separated string
    ...
    def get_sorted_log_data(self, sort_by, sort_order="asc"):
        ...
        for row in sorted_rows:
            ...
            for k, v in row.items():
                ...
                if new_key == "parentId":
                    try:
                        ...
                        new_row[new_key] = v
                elif new_key in ["value", "shock"] and isinstance(
                    casted_value, (int, float)
                ):
            ...
            new_row["iteration"] = (
                int(iteration) if str(iteration).isdigit() else iteration
            )
        ...
        try:
            ...
            for iteration, group in df.groupby("Iteration"):
                ...
                if country_filter_set is not None:
                    ...
                    group = group[
                        (group["Source"].isin(nodes))
                        & (group["Destination"].isin(nodes))
                    ]
